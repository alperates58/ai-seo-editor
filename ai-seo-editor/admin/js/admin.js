...
(function () {
	...
	function initAutoPublisherEnhanced() {
		...
		if (refreshBtn) {
			refreshBtn.addEventListener('click', async () => {
				...
				try {
					...
					renderQueue(queue);
					updateAllKpis(total, res.data?.stats || null);
		...
		function updateQueueKpi(total) { const el = document.querySelector('[data-kpi="queue-count"]'); if (!el) return; const safeTotal = Number.isFinite(total) ? Math.max(0, total) : 0; el.textContent = String(safeTotal); el.setAttribute('data-counter-target', String(safeTotal)); }
		function updateAllKpis(total, stats) { updateQueueKpi(total); if (!stats) return; const set = (slug, val, mini) => { const el = document.querySelector('[data-kpi="' + slug + '"]'); if (el) { el.textContent = String(val); } const me = document.querySelector('[data-kpi-mini="' + slug + '"]'); if (me && mini) me.textContent = mini; }; const s = stats; const avgSeo = s.avg_seo || 0; const avgRead = s.avg_readability || 0; const traffic = s.estimated_traffic || 0; set('published-today', s.published_today || 0, s.published_today > 0 ? 'Otomasyon aktif ilerliyor' : 'Yeni publish bekleniyor'); set('avg-seo', avgSeo ? avgSeo + '/100' : '--', avgSeo >= 80 ? 'Guclu optimizasyon' : 'Iyilestirme alani var'); set('avg-read', avgRead ? avgRead + '/100' : '--', avgRead >= 70 ? 'Akici icerik' : 'Daha sade metin gerekir'); set('failed', s.failed_count || 0, s.failed_count > 0 ? 'Esik altinda bekleyenler var' : 'Hata sinyali yok'); set('traffic', traffic > 0 ? '~' + traffic.toLocaleString('tr-TR') : '--', null); }
	}
